...
OVERRIDABLE_VARS=(
    ...
    DRY_RUN
    NON_INTERACTIVE
    VIRT_TUNING_BEHAVIOR
    HW_LOW_TIER_MAX_RAM_MB
    HW_LOW_TIER_MAX_CPU_CORES
    HW_MID_TIER_MAX_RAM_MB
    HW_MID_TIER_MAX_CPU_CORES
    MINECRAFT_USER
    ...
    MINECRAFT_QOL_MODS
    MINECRAFT_MODPACK_SOURCE
    MINECRAFT_MODPACK_SLUG
    MRPACK_INSTALL_VERSION
...
# Safely set a dynamic variable name (no export — variável fica local ao shell).
set_config_var() {
    local __key="$1"
    local __val="$2"
